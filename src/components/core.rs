...
impl DepthLayer {
    pub fn depth_value(&self) -> f32 {
        match self {
            DepthLayer::AlwaysBottom(local) => {
                *DEPTH_ALWAYS_BOTTOM + (*local as f32)
            }
            DepthLayer::Background(local) => {
                *DEPTH_BACKGROUND + (*local as f32)
            }
            DepthLayer::Ground(local) => *DEPTH_GROUND + (*local as f32),
            DepthLayer::Environment(local) => {
                *DEPTH_ENVIRONMENT + (*local as f32)
            }
            DepthLayer::Entities(local) => *DEPTH_ENTITIES + (*local as f32),
            DepthLayer::Effects(local) => *DEPTH_EFFECTS + (*local as f32),
            DepthLayer::Foreground(local) => {
                *DEPTH_FOREGROUND + (*local as f32)
            }
