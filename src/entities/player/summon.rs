...
use crate::components::{
    behavior::FollowPlayer, core::DepthLayer, markers::Player,
};
...
use crate::core::{
    animation::create_animation, extensions::EntityBuilderExt, make_spritesheet,
};
use crate::entities::player::state::MovingDirection;
use crate::modules::health::{DamageType, Health, Mana, Resistances};
use crate::modules::value_bar::{
    ValueBarColors, ValueBarConfig, spawn_value_bar,
};

use super::state::{
    ANIMATIONS, CAMERA_FOLLOW_SMOOTHNESS, HEALTH_BAR_OFFSET_Y,
    MANA_BAR_OFFSET_Y, PLAYER_COLLIDER_HALF_HEIGHT, PLAYER_COLLIDER_HALF_WIDTH,
    PLAYER_COLLIDER_OFFSET_X, PLAYER_COLLIDER_OFFSET_Y, PlayerAnimation,
    PlayerLogicFlags, PlayerStateHandler, SPRITESHEET, TILE_SIZE,
};
...
pub fn summon(
    ...
    let _scope = ProfileScope::new(
        &*profiling,
        "entities::player::summon::summon",
        &["player", "setup", "spawn", "entity"],
    );

    debug_log.add(&["player", "setup"], "Player summon started");
    ...
    let walk_config = anim_configs
        .iter()
        .find(|c| c.name == "walk")
        .expect("Missing 'walk' animation");
    let idle_config = anim_configs
        .iter()
        .find(|c| c.name == "idle")
        .expect("Missing 'idle' animation");

    let walk_handler =
        create_animation(&spritesheet, &mut animations, walk_config);
    let idle_handler =
        create_animation(&spritesheet, &mut animations, idle_config);
    ...
    let health_bar_config =
        ValueBarConfig::new().with_offset(0.0, *HEALTH_BAR_OFFSET_Y);
    ...
    debug_log.add(&["player", "setup"], "Player spawned");
}
