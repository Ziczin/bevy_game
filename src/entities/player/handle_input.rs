use bevy::prelude::*;
use crate::{components::markers::Player, core::debug_log::DebugLogBuffer, core::profiling::{ProfilingBuffer, ProfileScope}};
...
pub fn handle_input(
    ...
    let _scope = ProfileScope::new(&profiling, "entities::player::handle_input::handle_input", &["player", "input", "keyboard"]);
    ...
    for (logic_flags, mut player_direction) in &mut player {
        if !logic_flags.contains(PlayerLogicFlags::CanMove) {
            if player_direction.0 != direction {
                debug_log.add(&["player"], format!("Player input: direction changed to ({:.2}, {:.2})", direction.x, direction.y));
            }
