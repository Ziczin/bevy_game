use super::state::{
    GRID_HEIGHT, GRID_WIDTH, ROTATION_SEED_X, ROTATION_SEED_Y,
    ROTATION_VARIANTS, TILE_SIZE, TILE_TEXTURE_PATH,
};
...
fn get_tile_rotation(x: usize, y: usize) -> Quat {
    let seed = ((x as i32 * *ROTATION_SEED_X + y as i32 * *ROTATION_SEED_Y)
        % *ROTATION_VARIANTS) as f32;
...
pub fn spawn_ground(
    ...
    let _scope = ProfileScope::new(
        &*profiling,
        "entities::playground::spawn_ground::spawn_ground",
        &["playground", "setup", "spawn", "tile", "ground"],
    );
    ...
    for y in 0..grid_height {
        for x in 0..grid_width {
            ...
            let mut transform =
                Transform::from_xyz(world_x, world_y, layer.depth_value());
