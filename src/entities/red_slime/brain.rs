...
use super::state::{
    EXPECTED_IDLE_FRAME, RedSlimeAnimation, RedSlimeLogicFlags, RedSlimeState,
    RedSlimeStateHandler, WALK_DISTANCE_END, WALK_DISTANCE_START,
...
pub fn brain(
    ...
    let _scope = ProfileScope::new(
        &*profiling,
        "entities::red_slime::brain::brain",
        &["red_slime", "brain", "state", "animation", "ai"],
    );
    ...
    ) in &mut slime_query
    {
        ...
        pathfinder.is_active =
            distance >= walk_distance_end && distance <= walk_distance_start;

        if pathfinder.is_active != was_active {
            debug_log.add(
                &["red_slime", "pathfinding"],
                format!(
                    "RedSlime {:?}: Pathfinder active: {} (distance: {:.1})",
                    entity, pathfinder.is_active, distance
                ),
            );
        }

        let collider_pos =
            get_collider_world_position(transform, children, &child_query);

        let can_maneuver =
            if matches!(sprite_sheet.progress.frame, 3 | 4 | 5 | 6 | 7 | 8 | 9)
            {
                logic_flags.insert(RedSlimeLogicFlags::CanMove);
                false
            } else {
                logic_flags.remove(RedSlimeLogicFlags::CanMove);
                true
            };
        ...
        if pathfinder.current_target.is_some() && pathfinder.is_active {
            if state_handler.set(RedSlimeState::Walk) {
                debug_log.add(
                    &["red_slime"],
                    format!("RedSlime {:?}: State -> Walk", entity),
                );
            ...
            if state_handler.set(RedSlimeState::Idle) {
                debug_log.add(
                    &["red_slime"],
                    format!("RedSlime {:?}: State -> Idle", entity),
                );
            ...
            if let Some(target) = pathfinder.current_target {
                if (target - collider_pos).length() < waypoint_arrival_threshold
                {
                    debug_log.add(
                        &["red_slime", "pathfinding"],
                        format!(
                            "RedSlime {:?}: Reached waypoint at ({:.1}, {:.1})",
                            entity, target.x, target.y
                        ),
                    );
                    pathfinder.current_waypoint += 1;
                    if pathfinder.current_waypoint < pathfinder.path.len() {
                        *velocity = if let Some(current_target) =
                            Some(pathfinder.path[pathfinder.current_waypoint])
                        {
