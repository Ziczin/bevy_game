...
pub fn brain(
    ...
    profile_scope!(&profiling, "entities::red_slime::brain::brain", &["red_slime", "brain", "state", "animation", "ai"]);
    ...
    for (
        ...
        if pathfinder.is_active != was_active {
            debug_log!(&mut debug_log, &["red_slime", "pathfinding"], "RedSlime {:?}: Pathfinder active: {} (distance: {:.1})", entity, pathfinder.is_active, distance);
        }
