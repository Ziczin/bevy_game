...
pub fn update_nav_grid_position(
    ...
    profile_scope!(&profiling, "core::navigation::grid_updater::update_nav_grid_position", &["pathfinding", "navgrid", "update", "spatial_query"]);
    let Some(mut grid) = grid else {
        debug_log!(&mut debug_log, &["pathfinding"], "update_nav_grid_position: NavGrid not ready");
        return;
    };
    let Ok(player_transform) = player_query.single() else {
        debug_log!(&mut debug_log, &["pathfinding"], "update_nav_grid_position: Player not found");
        return;
    };
    ...
    if distance.x.abs() > threshold || distance.y.abs() > threshold {
        ...
        debug_log!(&mut debug_log, &["pathfinding"], "NavGrid snapped to ({:.1}, {:.1}). Walkable: {}, Blocked: {}", grid.origin.x, grid.origin.y, walkable, blocked);
    }
}