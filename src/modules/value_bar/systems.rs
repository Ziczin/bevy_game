...
use super::config::{
    ANIMATION_DURATION, COLOR_TRANSPARENT, DELAY_BEFORE_ANIMATION,
    FADE_DURATION, VISIBILITY_TIMEOUT, ValueBarConfig,
};
...
pub fn update_delayed_value(
    ...
    let _scope = ProfileScope::new(
        &*profiling,
        "modules::value_bar::systems::update_delayed_value",
        &["ui", "value_bar", "animation", "delayed"],
    );
    ...
    for mut bar in &mut bars {
        ...
        if bar.delay_timer > 0.0 {
            ...
            bar.delayed_value =
                bar.delayed_value.lerp(bar.target_delayed_value, t);
...
pub fn update_value_bar_visuals(
    ...
    let _scope = ProfileScope::new(
        &*profiling,
        "modules::value_bar::systems::update_value_bar_visuals",
        &["ui", "value_bar", "render", "visual"],
    );
    ...
    for bar in &mut bars {
        ...
        if let Ok((mut bg_sprite, mut bg_transform)) =
            sprites.get_mut(bar.background_entity)
        {
            bg_sprite.custom_size = Some(Vec2::new(
                bar_width + padding * 2.0,
            ...
            bg_transform.translation =
                Vec3::new(bar.offset_x, bar.offset_y, 0.0);
        }

        if bar.delayed_value > current_ratio {
            if let Ok((mut delayed_sprite, mut delayed_transform)) =
                sprites.get_mut(bar.delayed_entity)
            {
                ...
                delayed_transform.translation =
                    Vec3::new(bar_left + width / 2.0, bar.offset_y, 0.1);
            }

            if let Ok((mut current_sprite, mut current_transform)) =
                sprites.get_mut(bar.current_entity)
            {
                ...
                current_transform.translation =
                    Vec3::new(bar_left + width / 2.0, bar.offset_y, 0.2);
            }
        } else {
            if let Ok((mut delayed_sprite, mut delayed_transform)) =
                sprites.get_mut(bar.delayed_entity)
...
pub fn update_value_bar_visibility(
    ...
    let _scope = ProfileScope::new(
        &*profiling,
        "modules::value_bar::systems::update_value_bar_visibility",
        &["ui", "value_bar", "visibility", "fade"],
    );
    ...
    for mut bar in &mut bars {
        if bar.visibility_timer > 0.0 {
            ...
            let entities = [
                ...
                (
                    bar.delayed_entity,
                    if bar.delayed_value > bar.value {
